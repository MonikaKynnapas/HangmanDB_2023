...
public class Database {
    ...
    private void checkDatabase() {
        ...
        if (!f.exists()) { // databasefile not exist create table
            createTable(); // create words table
            createTableScores(); // create scores table
        } else { // databasefile exist
            if(!tableExist("words")) { // if table words does not exist
                createTable();
            }
            if(!tableExist("scores")) { // if table words does not exist
                createTableScores();
            }
        }
    }

    private boolean tableExist(String databaseTable) {
        try {
            ...
            return rs.getRow() > 0; // if > 0 table exist (true), null or 0 not exist (false)
    ...
    public  List<DatabaseData> selectById(int id) {
        ...
        try {
            ...
            ResultSet rs = ps.executeQuery();
            rs.next();
            databaseData.add(new DatabaseData(rs.getInt("id"),
                    rs.getString("word"),
